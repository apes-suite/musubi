...
damp_factor = 1.0 -- Maximum limit: 4.0/omega-0.001
...
-- box and radial.
source = {
  absorb_layer = 'absorblayer_plane',
    ...
    st_fun = {
      -- Bottom plane
      {
        ...
        spatial = {
          ...
          origin = {0.0, -length/2.0+abs_thickness+dx/2.0, 0.0},
          normal = {0.0,-1.0,0.0},
        ...
        shape = {
          kind = 'canoND',
          object = {
            origin = {-length/2.0+abs_thickness-dx, -length/2.0, dx/2.0},
            vec = {
              {length-2*(abs_thickness-dx), 0.0, 0.0},
              {0.0, abs_thickness+dx, 0.0}
            }
          }
        },
      },
      ...
      -- West plane
      {
        ...
        spatial = {
          predefined = 'spongelayer_plane',
          origin = {-length/2.0+abs_thickness+dx/2.0, 0.0, 0.0},
          normal = {-1.0,0.0,0.0},
          damp_profile = 'polynomial_n5',
          thickness = abs_thickness,
          damp_factor = damp_factor,
...
--  {
--    label = 'vtk',
--    variable = {'pressure_phy', 'absorblayer_plane',
...
--    output = {format='vtk'},
--    folder='tracking/',
--    time_control = {interval=tmax/30, min={iter= 0}, max = tmax}
--  }
}
