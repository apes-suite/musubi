...
#  os.remove(dbname)
  ...
  # Load tracking output with pressAlongLength at t=0s and t=1s for level 5
  sqlcon = gleaner.tracking_to_db(fname = ['./reference/*L5*pressAlongLength*t0*.res'], \
                                  dbname=dbname, tabname='press_line_t0_L5')

  # Load tracking output with pressAlongLength at t=0s and t=1s for level 6
...
x, y = zip(*sorted(zip(x, y)))
# Plot simulation result
mplt.plot(x, y, '-x', color = 'b', label = 'L6')
