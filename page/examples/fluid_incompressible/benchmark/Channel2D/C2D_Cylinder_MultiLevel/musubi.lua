...
vel_lat = Ma * cs_lat
-- Physical time step computed from physical and lattice speed of sound
dt = dx * cs_lat / cs_phy
--------------------------------------------------------------------------------
...
boundary_condition = {
  {
    ...
    velocity = {
      ...
      temporal  = {
        ...
        from_time = 0,
        to_time = 1.0 -- ramp up to 1.0 sec
      },
      spatial = vel_inflow
...
tracking = {
  -- Output file to visualize in Paraview.
  {
    ...
    variable = { 'pressure_phy','velocity_phy', 'vorticity_phy',
                 'q_criterion_phy' },
    shape = { kind = 'all' },
    time_control = { min = tmax_phy, max = tmax_phy, interval = 1.0/25 },
    output = { format = 'vtk' }
  },
  ...
  -- Track pressure before cylinder over time.
  {
    ...
    shape = {
      kind = 'canoND',
      object = {
        origin = { x_offset + 0.15-dx_cyl/2, y_offset + 0.2, dx/2.0 }
      }
    },
    time_control = {
      min = trac_start,
  ...
  -- Track pressure after cylinder over time.
  {
    ...
    shape = {
      kind = 'canoND',
      object = {
        origin = { x_offset + 0.25+dx_cyl/2, y_offset + 0.2, dx/2.0 }
      }
    },
    time_control = {
      min = trac_start,
  ...
  -- Track drag and lift coefficient on cylinder over time
  {
    ...
    time_control = {min = trac_start, max = tmax_p, interval = 10*dt},
    reduction = {'sum', 'sum'},
    -- ascii format writes variable values and simulation time into a single
    -- file at every tracking interval. It is usally used for point tracking.
    output = {format = 'ascii'}
