...
-- Speed of sound [m/s]
cs_phy = 343.0
-- Inflow velocity at center of pipe computed from Machs number [m/s]
vel_phy = Ma * cs_phy
...
-- Pressure drop across the channel length for Pipe flow
...
-- Second reference for analytical solution:
-- http://www.ucdenver.edu/faculty-staff/jguo/Documents/Fluid1/11Laminar.pdf
...
tracking = {
  {
    label = 'vtk',
    folder = tracking_fol,
    variable = { 'pressure_phy', 'velocity_phy', 'vel_analy' },
    shape = {kind = 'all'},
    time_control = {
      min = trac_start,
      max = tmax_phy,
  ...
  },
  {
    ...
    shape = {
      kind = 'canoND',
      object = {
        origin = { dx/2.0, pipe_origin[2], pipe_origin[3] },
        vec = {
          { 0.0, diameter, 0.0 },
          { 0.0, 0.0, diameter }
        }
  ...
  },
  {
    ...
    shape = {
      kind = 'canoND',
      object = {
        origin = { length-dx/1.0, pipe_origin[2], pipe_origin[3] },
        vec = {
          { 0.0, diameter, 0.0 },
          { 0.0, 0.0, diameter }
        }
  ...
  },
  {
    ...
    shape = {
      kind = 'canoND',
      object = {
        origin = { 0.0, pipe_center[2], pipe_center[3] },
        vec = { length, 0.0, 0.0 },
        segments = nLength
      }
    },
  ...
  { 
    ...
    shape = {
      kind = 'canoND',
      object = {
        origin = { length*0.5+dx/2.0, pipe_origin[2], pipe_center[3] },
